...
export default function AdminShellLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (user && !user.is_admin) router.replace(`/${locale}`);
  ...
  const links = useMemo(() => [
    ...
    { label: t("options") || "Options", href: `/${locale}/admin/options`, icon: Sliders },
    { label: t("orders"), href: `/${locale}/admin/orders`, icon: Receipt },
    { label: t("cities"), href: `/${locale}/admin/cities`, icon: MapPin },
    { label: t("banners"), href: `/${locale}/admin/banners`, icon: LayoutTemplate },
  ], [t, locale]);
  ...
  const SidebarContent = () => (
    <div className="h-full flex flex-col" style={{ background: "#0F172A", color: "rgba(255,255,255,0.72)" }}>
      {/* Logo */}
      <div className="flex items-center gap-4 px-6 border-b border-white/5" style={{ height: 64 }}>
        ...
        <div className="min-w-0">
          ...
          <p className="text-[0.7rem] text-white/40 tracking-wide">{t("adminPanel")}</p>
        </div>
      </div>

      {/* Nav */}
      <div className="flex-1 overflow-y-auto px-3 py-3">
        <p className="px-3 text-[0.68rem] font-bold tracking-widest uppercase text-white/35 mb-2">{t("mainMenu")}</p>
